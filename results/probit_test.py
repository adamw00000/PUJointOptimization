        ...
        for i in range(n_runs):
            ...
            while not has_res:
                try:
                    ...
                    for clf_name, clf in [
                        ('Oracle', OracleClassifier(include_bias=False)),
                        ...
                        if clf_name == 'DCCP':
                            ...
                            results.append({
                                ...
                                'RiskBHat': joint_risk(clf.params, X_train, s_train, c),
                                'RealProbaError': np.mean(np.abs(est_probas - real_probas)),
                                'OracleProbaError': np.mean(np.abs(est_probas - oracle_probas)),
...
for i, target_c in enumerate(df.c.unique()):
    ...
    methods = [m for m in methods if m not in ['Oracle', 'Naive']] + ['Oracle']
    for method in methods:
        ...
        error_ax.set_ylim(bottom=-200, top=5200)
        ...
        angle_ax.set_ylim(bottom=0, top=8.4)
        ...
        real_proba_ax.set_ylim(bottom=-0.01, top=0.26)
        ...
        oracle_proba_ax.set_ylim(bottom=-0.01, top=0.26)
        ...
        length_ax.set_ylim(bottom=10**(-0.25), top=10**3.8)
...
for fig, name, suptitle in [
    ...
    (angle_fig, 'angle', 'Probit - kąt pomiędzy rzeczywistym estymowanym a rzeczywistym wektorem parametrów'),
