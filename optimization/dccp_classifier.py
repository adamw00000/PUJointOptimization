...
class DccpClassifier(SplitOptimizationPUClassifier):
    ...
    def __init__(self, tol: float = 1e-4, max_iter: int = 100, dccp_max_iter: int = 20, tau: float = 1,
                 verbosity: int = 0):
        super().__init__('DCCP', tol=tol, max_iter=max_iter, max_inner_iter=dccp_max_iter, verbosity=verbosity)
        self.tau = tau

    @staticmethod
    def __build_problem(X, s, c_estimate):
        X = add_bias(X)
        n = X.shape[0]
        n_params = X.shape[1]
    ...
    def _minimize_wrt_b(self, X, s, c_estimate, old_b_estimate) -> npt.ArrayLike:
        problem, b = self.__build_problem(X, s, c_estimate)

        # print(problem.is_dcp(), dccp.is_dccp(problem))
        result = problem.solve(method='dccp', tau=self.tau, solver=cvx.ECOS, max_iter = self.max_inner_iter,
